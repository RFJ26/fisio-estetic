...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        $erro_msg = "Não é possível apagar: O cliente tem marcações associadas.";
    }
}
...
if(isset($_GET['search']) && !empty(trim($_GET['search']))){
    ...
    // Pesquisa também por NIF nos clientes
    $where = "WHERE nome LIKE '%$search%' OR email LIKE '%$search%' OR telefone LIKE '%$search%' OR nif LIKE '%$search%'";
}
...
// ============================================================================
$query = "SELECT id, nome, email, telefone, nif, email_verificado 
          FROM cliente 
          $where 
          ORDER BY nome ASC 
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <div class="header-actions d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h1 class="h3 mb-1 fw-bold text-dark">Clientes</h1>
                    <p class="text-muted mb-0">Gestão e consulta de clientes registados.</p>
                </div>
                ...
                <form class="row g-3 align-items-center" action="" method="GET" id="form-pesquisa">
                    <div class="col-12">
                        <div class="search-box">
                            ...
                            <input type="text" id="campo-pesquisa" name="search" autocomplete="off" placeholder="Procurar cliente por nome, email, nif..." value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>">
                        </div>
                    </div>
                        ...
                        <tbody>
                            <?php if(mysqli_num_rows($clientes) > 0): ?>
                                <?php while($cliente = mysqli_fetch_assoc($clientes)): 
                                    $nomes = explode(" ", trim($cliente['nome']));
                                ...
                                <tr>
                                    ...
                                    <td data-label="Ações" class="text-end pe-4">
                                        <div class="action-buttons justify-content-end">
                                            ...
                                            <a href="edit.php?id=<?= $cliente['id'] ?>" class="btn-icon btn-edit" title="Editar">
                                                <i class="bi bi-pencil-square"></i>
                                            </a>
                                            <a href="list.php?delete=<?= $cliente['id'] ?>" class="btn-icon btn-delete" title="Apagar" onclick="return confirm('Tem a certeza que deseja apagar este cliente?');">
                ...
                timer = setTimeout(() => {
                    ...
                    fetch(url)
                        ...
                        .then(html => {
                            ...
                            // Extrai e substitui a tabela e a paginação novas
                            ...
                            // Atualiza a URL no browser sem recarregar a página
