...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        $erro_msg = "Não é possível apagar: Este serviço tem marcações associadas.";
    }
}
...
$query_count = "SELECT COUNT(*) as total FROM servico $where";
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <div class="header-actions d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h1 class="h3 mb-1 fw-bold text-dark">Serviços</h1>
                    <p class="text-muted mb-0">Gestão de tratamentos e preçário.</p>
                </div>
                
                <a href="create.php" class="btn-add">
                    <i class="bi bi-plus-lg"></i> <span class="d-none d-sm-inline">Novo Serviço</span>
                </a>
            </div>
                ...
                <form class="row g-3 align-items-center" action="" method="GET" id="form-pesquisa">
                    <div class="col-12">
                        <div class="search-box">
                            ...
                            <input type="text" id="campo-pesquisa" name="search" autocomplete="off" placeholder="Procurar serviço..." value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>">
                        </div>
                    </div>
            ...
            <?php if(isset($_GET['msg']) && $_GET['msg'] == 'deleted'): ?>
                <div class="alert alert-success alert-dismissible fade show shadow-sm" id="successAlert">
                    <i class="bi bi-check-circle me-2"></i> Serviço removido com sucesso.
            ...
            <div class="table-container" id="conteudo-tabela">
                <div class="table-responsive">
                    <table class="table custom-table mb-0">
                        <thead>
                            <tr>
                                <th width="35%">Designação</th>
                                <th width="20%">Categoria</th>
                                <th width="15%" class="text-center">Duração</th>
                                <th width="15%" class="text-center">Preço</th>
                                ...
                                <tr>
                                    ...
                                    <td data-label="Categoria">
                                        <?php if(!empty($row['categoria_nome'])): ?>
                                            <span class="category-tag"><?= htmlspecialchars($row['categoria_nome']) ?></span>
                                        <?php else: ?>
                                            <span class="text-muted small">-</span>
                                        <?php endif; ?>
                                    </td>
                                    <td data-label="Duração" class="text-center">
                                        <span class="slots-badge">
                                            <i class="bi bi-clock me-1"></i> <?= converterSlotsParaTempo($row['num_slots']) ?>
                                        </span>
                                    </td>
                                    ...
                                    <td data-label="Ações" class="text-end pe-4">
                                        <div class="action-buttons">
                                            <a href="view.php?id=<?= $row['id'] ?>" class="btn-icon btn-view" title="Ver Detalhes"><i class="bi bi-eye"></i></a>
                                            <a href="disponibilidade_create.php?id_servico=<?= $row['id'] ?>" class="btn-icon btn-schedule" title="Horários"><i class="bi bi-calendar-week"></i></a>
                                            <a href="edit.php?id=<?= $row['id'] ?>" class="btn-icon btn-edit" title="Editar"><i class="bi bi-pencil-square"></i></a>
                                            <a href="list.php?delete=<?= $row['id'] ?>" class="btn-icon btn-delete" onclick="return confirm('Apagar serviço?')" title="Apagar"><i class="bi bi-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="5" class="text-center py-5 text-muted">Nenhum serviço encontrado.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
