...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['acao']) && $_POST['acao'] === 'adicionar') {
        ...
        if ($slot_inicial >= $slot_final) {
            ...
            // Inserir na base de dados (Sem abreviaturas SQL)
            $query_insert = "
            ...
            if (mysqli_query($conn, $query_insert)) {
                // Cancelar marcações concorrentes usando nomes completos das tabelas
                $query_cancelar = "
                    UPDATE marcacao
                    ...
                    AND marcacao.estado IN ('ativa', 'por confirmar', 'pendente')
...
// =========================================================================
// 3. CONSULTA SQL DE INDISPONIBILIDADES
// =========================================================================
$query_indis = "
...
<body>
    ...
    <nav class="sidebar d-flex flex-column">
        ...
        <ul class="nav flex-column mt-4">
            ...
            <li class="nav-item"><a class="nav-link active" href="indisponibilidade.php"><i class="bi bi-slash-circle me-3"></i> Indisponibilidade</a></li>
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <div class="header-actions d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h1 class="h3 mb-1 fw-bold text-dark">Indisponibilidade</h1>
                    <p class="text-muted mb-0">Gerir ausências e bloqueios de agenda.</p>
                </div>
                <button class="btn-add d-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#modalAdicionar">
                        ...
                        <tbody>
                            <?php if($resultado_indis && mysqli_num_rows($resultado_indis) > 0): ?>
                                <?php while($item = mysqli_fetch_assoc($resultado_indis)): ?>
                                    ...
                                    ?>
                                    <tr>
                                        <td data-label="Período / Dias">
                                            <div class="date-day">
                                                <?= date('d/m/Y', strtotime($item['data_inicio'])) ?>
                                                <?php if(!$mesmo_dia): ?>
                                                    <small>a <?= date('d/m/Y', strtotime($item['data_fim'])) ?></small>
                                                <?php endif; ?>
                                            </div>
                                            <span class="days-text text-muted small mt-1 d-block"><i class="bi bi-calendar-week me-1"></i> <?= $todos_os_dias ?></span>
                                        </td>
                                        <td data-label="Horário">
                                            <div class="date-time">
                                                <i class="bi bi-clock me-1"></i><?= $hora_inicio_vis ?> <i class="bi bi-arrow-right mx-1"></i> <?= $hora_fim_vis ?>
                                            </div>
                                        </td>
                                        ...
                                        <td data-label="Ações" class="text-end pe-4">
                                            <div class="action-buttons">
                                                <form method="POST" action="" onsubmit="return confirm('Tem a certeza que deseja libertar este bloqueio? Esta ação não pode ser desfeita.');" class="m-0 p-0">
                                                    <input type="hidden" name="acao" value="remover">
                                                    <input type="hidden" name="id_bloqueio" value="<?= $item['id'] ?>">
                                                    <button type="submit" class="btn-icon btn-cancel" title="Remover Bloqueio">
            ...
            <div class="modal-content border-0 shadow-lg" style="border-radius: 16px;">
                <div class="modal-header border-bottom-0 pb-0 mt-3 mx-3">
                    <h5 class="modal-title fw-bold text-dark d-flex align-items-center gap-2">
                        <i class="bi bi-calendar-minus" style="color: var(--primary-color);"></i> Nova Indisponibilidade
                    </h5>
                ...
                <form method="POST" action="">
                    ...
                    <div class="modal-footer border-top-0 pt-0 pb-4 px-4 gap-2">
                        <button type="button" class="btn btn-light fw-medium flex-grow-1" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" class="btn btn-primary flex-grow-1 m-0" style="background-color: var(--primary-color); border: none;">Confirmar</button>
                    </div>
                </form>
            </div>
        </div>
